...
namespace EqCrm.Controllers.CargaFacturacionMasiva
{
    public class FacturacionMasivaController : Controller
    {
        ...
        [HttpPost]
        public string AnularFacturas(string informacion)
        {
            string cUserConected = (string)(Session["Usuario"]);
            string str = "";
            ...
            try
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    string nofactura = row["NOFACTURA"].ToString();
                    ...
                    if (nofactura != "No_Factura")
                    {

                        cInst =  "SELECT fecha_certificacion,firmaelectronica,nit,id_cliente  FROM facturas WHERE no_factura = "+nofactura;
                        cInst += " AND serie  ='"+serie+"'";
                        ...
                        var wsEnvio = wsConnector.wsEnvio(Funciones.cCadenaJson(json, "firmaelectronica"), "", "XML", cUserFe, cUrlFel, cToken, "GET_DOCUMENT", "GT", cNit, false, "");

                        string cXmlAnulacion = CrearXml.CreaDteAnulaFact(Funciones.cCadenaJson(json, "firmaelectronica"), wsEnvio[7] , nit, cNit, cXmlAnula, "NO PROCEDE");

                        var wsAnula = wsConnector.wsEnvio("VOID_DOCUMENT", Funciones.Base64Encode(cXmlAnulacion), "", cUserFe, cUrlFel, cToken, "SYSTEM_REQUEST", "GT", cNit, false, "");

                        var lProcesa = Funciones.ProcesarAnulacion(nofactura.ToString(), serie.ToString(), Funciones.cCadenaJson(json, "id_cliente"), "facturas", DB.ToString(), cUser);
                ...
                cInst = "SELECT no_factura as NOFACTURA,serie AS SERIE,fecha AS FECHA,id_cliente CODIGO, cliente AS CLIENTE, nit AS NIT, status AS STATUS, direccion AS DIRECCION, email AS MAIL, total AS TOTAL, firmaelectronica AS AUTORIZACION, no_docto_fel AS DOCTO, serie_docto_fel AS SERIEFEL, error AS ERROR FROM `facturas` WHERE fecha_nota IN(curdate())";


                var resultados2 = eqdap.ExecuteList<dynamic>(DB, cInst);

                var json2 = JsonConvert.SerializeObject(resultados2);
