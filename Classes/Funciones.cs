...
namespace EqCrm
{
    public class Funciones
    {
        ...
        public static bool ProcesarAnulacion(string docto,string serie, string idcliente, string ctabla,string oDb, string cUser )
        {
            ...
            cInst = "UPDATE " + ctabla + " SET fecha_nota = '"+ DateTime.Now.ToString("yyyy-MM-dd") + "' WHERE no_factura = @NoFactura AND serie = @Serie";

            // Creamos los parametros para factura
            object[] parametrosfactura2 = { new { NoFactura = docto, Serie = serie } };
            lError = eqdaper.EqExecuteParam(cInst, oDb, ref cError, parametrosfactura2);
            if (lError == true)
            {
                return false;
            }
