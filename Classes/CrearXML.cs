...
namespace EqCrm
{
    public static class CrearXml
    {
        ...
        public static string CreaDteAnulaFact(string UUID, string FechaEmision, string cNitreceptor, string cNitEmisor, string cXmlAnula,string cMotivoAnula)
        {
            ...
            dt = Convert.ToDateTime(FechaEmision);

            cDteAnula = cXmlAnula;
            cDteAnula = cDteAnula.Replace("{FECHAEMISION}", dt.ToString("yyyy-MM-ddTHH\\:mm\\:ss"));
            cDteAnula = cDteAnula.Replace("{FECHAANULACION}", DateTime.Now.ToString("yyyy-MM-ddTHH\\:mm\\:ss"));
